...
use KratosUpdateChecker\UpdateChecker;
...
if (!class_exists(Panel::class, false) && class_exists('Debug_Bar_Panel', false)):
    ...
    class Panel extends \Debug_Bar_Panel
    {
        ...
        private $responseBox = '<div class="ajax-response" style="display: none;"></div>';

        public function __construct($updateChecker)
        {
            $this->updateChecker = $updateChecker;
            $title = sprintf(
                '<span class="debug-menu-link-%s">PUC (%s)</span>',
        ...
        {
            printf(
                '<div class="debug-bar-panel-v5" id="%1$s" data-slug="%2$s" data-uid="%3$s" data-nonce="%4$s">',
                esc_attr($this->updateChecker->getUniqueName('debug-bar-panel')),
                esc_attr($this->updateChecker->slug),
                esc_attr($this->updateChecker->getUniqueName('uid')),
                esc_attr(wp_create_nonce('ajax'))
            );
        ...
        private function displayStatus()
        {
            ...
            if (function_exists('get_submit_button')) {
                ...
                $checkNowButton = sprintf(
                    '<input type="button" id="%1$s" name="check-now-button" value="%2$s" class="button button-secondary" />',
